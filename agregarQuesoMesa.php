<?php
	...
	if(isset($_POST['qma'])){
		...
		$tjs = mysqli_query($con,"select * from puntaje p
									where id_mesa = $id_m
									and id_jurado <> 1 
									group by p.id_jurado");

		while($tj = mysqli_fetch_array($tjs)){

			$id_j = $tj['id_jurado'];
			$id_m_p = $tj['id_muestra_puntaje'];
			...
			$roque = mysqli_query($con,"select c.descripcion_categoria
				...
				switch($nom_q[0]){
					...
					$insert_concensuada = "insert into puntaje(id_mesa,id_queso,id_jurado,ai_3,ai_4,ai_5,ai_6,$campos) 
										values ($id_m,$id_q,1,null,null,null,null,$nom_campos_1)";
