			...
			$quesos = mysqli_query($con,"select q.id_queso 'id', q.codigo_queso, c.descripcion_categoria 'nombre', (ae_1+ae_2+ai_1+ai_2+ai_7) 'pa'
										...
										,(ae_1+ae_2+ai_1+round(((ai_2+ai_3+ai_4+ai_5+ai_6)/5),2)+ai_7+flavor_1+flavor_2+flavor_3+flavor_4+flavor_5
										 ...
										 ,c.codigo_categoria 'cc', t.descripcion_tipo 'dt'
										  ...
										  and id_jurado=".$_SESSION['id_jurado'].			
										  " group by q.id_queso");
			//var_dump($result2);
			...
			while($queso = mysqli_fetch_array($quesos)){
				...
				echo "<table class='data'>";
					if ($queso['nombre'] != 'PASTA SEMIDURA CON OJOS'){
						echo "<tr><th colspan='4'><a style='color:#82000A' href='planilla.php?id_queso=".$queso['id']."'>".$queso['cc']." - ".$queso['dt']." - ".$queso['codigo_queso']."</a></th></tr>";
					}
					else{
						echo "<tr><th colspan='4'><a style='color:#82000A' href='planillaDos.php?id_queso=".$queso['id']."'>".$queso['cc']." - ".$queso['dt']." - ".$queso['codigo_queso']."</a></th></tr>";	
					}
					echo "<th>Apariencia</th> 
					<th>Flavor</th>
					<th>Textura</th>
					<th>Total</th>
					<tr>";
				if ($queso['nombre'] == 'PASTA SEMIDURA CON OJOS'){
					echo "<td>".$queso['par']."</td>";
				}	
				else{
					echo "<td>".$queso['pa']."</td>";
				}
